...
const EditFiscalYear = () => {
  ...
  const fiscalYearId = window.location.pathname.split('/').pop();
  const { data, loading: fetching } = useFetchObject({
    queryKey: ['fiscal-year', fiscalYearId],
    endpoint: `fiscal-years/${fiscalYearId}/`,
  });
  ...
  const handleSubmit = async () => {
    ...
    handleUpdate(fiscalYearId, formData);
  };
    ...
    <div className="container mx-auto py-6 space-y-6 max-w-4xl">
      <div className="flex items-center gap-4">
        ...
        <div className="flex items-center gap-2">
          <h1 className="text-base font-boldtext-sm">{t('accounting.editFiscalYear')}</h1>
        </div>
      </div>
