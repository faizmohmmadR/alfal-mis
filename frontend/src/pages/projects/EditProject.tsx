...
const EditProject = () => {
  ...
  const { data: project, isLoading: projectLoading } = useFetchObjects({
    queryKey: ['project', id],
  ...
  useEffect(() => {
    if (project) {
      setFormData({
        ...
        paid_amount: project.paid_amount?.toString() || '0',
        currency: project.currency || 'AFN',
        contract_date: project.contract_date || new Date().toISOString().slice(0, 10),
              ...
              <div className="p-4 bg-blue-50 border border-blue-200 rounded-lg">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  <div className="flex justify-between items-center p-2 bg-white rounded border">
                    ...
                    <span className="font-bold text-green-600">{project.budget} {project.currency}</span>
                  </div>
                  <div className="flex justify-between items-center p-2 bg-white rounded border">
                    <span className="text-gray-600 font-medium">{t('projects.totalPaid')}:</span>
                    <span className="font-bold text-blue-600">{project.paid_amount} {project.currency}</span>
                  </div>
                  <div className="flex justify-between items-center p-2 bg-white rounded border">
                    <span className="text-gray-600 font-medium">{t('projects.totalRemaining')}:</span>
                    <span className="font-bold text-orange-600">{project.remaining_amount} {project.currency}</span>
                  </div>
                </div>
              </div>
            )}
