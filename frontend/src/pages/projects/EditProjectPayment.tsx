...
const EditProjectPayment = () => {
  ...
  const { data: payment, isLoading } = useFetchObjects({
    queryKey: ['project-payment', id],
    endpoint: `project-payments/${id}/`,
  });

  const { handleUpdate, loading, isSuccess } = useUpdate<PaymentFormData>({
  ...
  useEffect(() => {
    if (payment) {
      setFormData({
        project: payment.project?.toString() || '',
      ...
      if (payment.project_details) {
        setSelectedProject(payment.project_details);
      }
    }
  }, [payment]);
                ...
                <Autocomplete
                  ...
                  onChange={(value, option) => {
                    setFormData(prev => ({ 
                      ...prev, 
                      project: value,
                      currency: option?.currency || prev.currency
                    }));
...
export default EditProjectPayment;
