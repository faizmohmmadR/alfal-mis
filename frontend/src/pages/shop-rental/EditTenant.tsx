...
const EditTenant = () => {
  ...
  const { handleUpdate, loading } = useUpdate({ queryKey: ['tenants'] });
  ...
  const handleSubmit = async () => {
    ...
    handleUpdate(tenantId, formData);
    navigate('/tenants');
  };
  ...
  return (
    <div className="container mx-auto py-6 space-y-6 max-w-4xl">
      <div className="flex items-center gap-4">
        ...
        <h1 className="text-base font-bold">{t('shop-rental.editTenant')}</h1>
      </div>
      ...
      <Card>
        <CardContent className="pt-6 space-y-4">
          ...
          <div className="flex justify-end gap-2">
            <Button variant="outline" onClick={() => navigate('/tenants')} disabled={loading}>{t('common.cancel')}</Button>
